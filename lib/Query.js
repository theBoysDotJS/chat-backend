...
class Query {
	...
	getSingleConversationMessages(conversationId, user_id, order, limit) {
		...
		var limit = typeof limit === 'undefined'
			? 200
			: limit;
		var m = knex.select("message.id", 'author', 'message_body', 'avatarUrl').from('message').join('user', 'user.id', '=', 'message.author').where('conversation_id', conversationId).orderBy('message.created_at', order).limit(limit).toString();
