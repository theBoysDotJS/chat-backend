...
class Query {
	...
	createUser(user) {
		...
		if (!user.avatarUrl) {
			...
			user.avatarUrl = `https://www.gravatar.com/avatar/${email}`
		}
	...
	getSingleConversationMessages(conversationId, user_id, order, limit) {
		...
		var limit = typeof limit === 'undefined'
			? 200
			: limit;
		var m = knex.select("message.id", 'author', 'message_body', 'user.username','avatarUrl').from('message').join('user', 'user.id', '=', 'message.author').where('conversation_id', conversationId).orderBy('message.created_at', order).limit(limit).toString();
	...
	joinConversationAllUsers(conversation_id, user) {
		console.log(conversation_id, user, 'THE JOIN VALUES<<<<<<<<')
