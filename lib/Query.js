...
const USER_FIELDS = ['id', 'email', 'username', 'password', 'firstName', 'lastName', 'language', 'avatarUrl'];
...
// Auth functions
 ...
 createUser(user) {
   console.log(user, 'this is inside create user')
   ...
   .then(hashedPassword => {
	console.log(hashedPassword, 'this is paddword hash')
     return this.query(
       knex
       .insert({
         email: user.email,
   ...
   .then((result) => {
	 console.log(result, 'this is the result from the first query')
     return this.query(
       knex
       .select(USER_FIELDS)
       .from('user')
   ...
   .then(result => {
	 console.log(result, 'this is the result from the final query')
   return result[0]
 })
   .catch((error) => {
 ...
 getUserFromSession(sessionToken) {
   console.log(sessionToken, 'inside getUser')
   return this.query(
     knex
   ...
   .then((result) => {
   console.log(result, 'the result inside get user')
     if (result.length === 1) {
       return result[0];
     }
     return null;
 ...
 createTokenFromCredentials(email, password) {
	console.log(email, password, 'these are creds')
   const errors = validate.credentials({
     email: email,
     password: password
   });
   if (errors) {
	 console.log(errors, 'this is validation error')
     return Promise.reject({ errors: errors });
   }
   console.log('past validation')
   let sessionToken;
   ...
   .then((results) => {
     console.log(results, 'first result in sessions')
     if (results.length === 1) {
       user = results[0];
	   console.log(password, user.password, 'brcypy results')
...
createNewConversation(conversation){
  console.log(conversation, "here we are in the funciton now..")
    return this.query(
      knex
      .insert(conversation)
      .into('conversation')
    ...
    .then(result => {
		  console.log(result, 'the result of the queries')
		  return result[0];
    })
    .catch((error) => {
      throw error;
